...
\section secIntroFunction Springheadの機能　

\ref pagePhysics としては，次の機能が使えます：
<ul>
 <li> 多数の剛体のシミュレーション
 <li> 凸多面体・直方体・球を扱うことができる接触判定
 <li> 跳ね返り係数，静止摩擦，動摩擦を扱うことができる接触力計算
 <li> 関節(蝶番，スライダ，ユニバーサルジョイント，ボールジョイント，パラメトリックジョイント)．
 <li> 安定なバネダンパモデル
</ul>
...
<ul>
 <li> \ref pageGraphics
 <ul>
  <li> 物理エンジンの状態をOpenGLを用いて表示するデバッグレンダリング．
  <li> モデラが出力したファイルを読み出して表示するグラフィックスシーン．
 </ul>
 <li> \ref pageFileIO シーングラフ（剛体，形状，関節など）の情報のロード・セーブ
</ul>
...
\section secHistory 歴史
Springheadは，長谷川晶一と田崎勇一が中心となって，
東工大精密工学研究所佐藤誠研の学生と共に開発してきました．
力覚インタフェースSPIDARと共に使える物理シミュレータとしてペナルティ法に基づくシミュレータSpringhead1を開発してきましたが，このたび安定性と高速性を重視してゲームのための物理エンジンや
バーチャルヒューマンの研究に使えるシミュレータとしてSpringhead2(このマニュアルで説明するSpringhead)を開発しています．

\section secIntroChar	Springheadの特徴
...
ここまでは，2006年現在，もっとも一般的な物理シミュレーション手法です．
