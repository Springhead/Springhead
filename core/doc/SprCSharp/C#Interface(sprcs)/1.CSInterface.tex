  ...
  \item	Debug構成でビルドすると\tt{SNAP}, \tt{DUMP}などが有効となり、
	...
	\begin{itemize}
	  ...
	  \item	\CPP 側で\FUNC{::SysAllocStringLen()}により割り当てたメモリを解放する
		コードは\\
		\hspace{20pt}\verb|#ifdef FREE\_UNMANAGED\_MEMORY| ～ \verb|#endif|\\
...
\begin{enumerate}
  ...
  \item	\FILE{cp.bat}を実行すると、これを\FILE{Springhead2/bin/swig/}にコピーする。
        このとき、ファイル名を\FILE{swig\_sprcs.exe}とリネームする。
	\begin{itemize}
	  \item	\FILE{cp.bat}の引数については\tt{cp.bat -h}を実行のこと。
	  \item	リネームする名前(\FILE{swig\_sprcs.exe})は\FILE{cp.bat}に
		ハードコードされている。
	\end{itemize}
...
\begin{enumerate}
  ...
  \item	\FILE{CSxxxx.wrapper.cs}\\
	...
	\tt{wrapper}クラスの命名規則は、
	\begin{narrow}
		\tt{vectorwrapper\_<\it{CLASS}>\_<\it{MEMBER}>}\\
		\tt{arraywrapper\_<\it{CLASS}>\_<\it{MEMBER}>}
	\end{narrow}
