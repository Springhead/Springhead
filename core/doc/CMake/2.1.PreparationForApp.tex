...
\fi
	...
	& \RBox{ビルドの対象とするファイル\\
		...
		相対パス指定となります。デフォルトは \\
		%\ \ {\footnotesize{\tt{file(GLOB SRCS \SetRelPath\ *.cpp *.h)}}} \\
	...
	& \RBox{\tt{SPRLIBTYPE}が\tt{STATIC}の場合 \\
		追加のライブラリファイル名 (\tt{-l}は不要です)} \\\HLine
    &\multicolumn{2}{l|}{\tt{STATIC\_LIBRARY\_NAMES\_ADD}} \\
	&& \RBox{\tt{SPRLIBTYPE}が\tt{SHARED}の場合 \\
		追加の静的ライブラリファイル名 (\tt{-l}は不要です)} \\\HLine
    &\multicolumn{2}{l|}{\tt{SHARED\_LIBRARY\_NAMES\_ADD}} \\
	&& \RBox{\tt{SPRLIBTYPE}が\tt{SHARED}の場合 \\
		追加の共有ライブラリファイル名 (\tt{-l}は不要です)} \\\HLine
    &\multicolumn{2}{l|}{\tt{EXCLUDE\_LIBRARY\_NAMES}} \\
	&& \RBox{リンクの対象から外すライブラリファイル名\\
		デフォルトで組み込まれてしまうライブラリファイルを
		排除するために指定します。} \\\HLine
    &\tt{VS\_VERSION}
	& \RBox{使用するVisual Studioのバージョン \\
		リンクする\SprLib{}ファイルを指定します。\\
		\bf{注意} ここで指定する値は、生成されるソリューションファイルの
		バージョンとは関係ありません。} \\\HLine
