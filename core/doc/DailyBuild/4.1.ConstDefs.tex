...
%\newpage
\subsection{定数定義クラス ConstDefs}
\label{subsec:ConstDefs}

\begin{Brief}[b]
	DailyBuildシステムで共通に使用する定数を提供するクラス。
\end{Brief}

\medskip
\begin{Description}{\bf{クラス \tt{CFK}}}
	\def\Row#1#2#3{\tt{#1}&\tt{#2}&\FCol{180pt}{#3}\\\hline}
	テスト制御ファイルで定義された\KQuote{キーワード}を参照するための
	定数を定義するクラス。
	次のものがある。
	...
	\begin{longtable}[r]{|l|l|l|}\hline
	    ...
	    \Row{EXPECTED}{Expected}{%
			テストプログラム終了コードの予想値\\
			テストの合否判定に使用する}
	    \Row{ADD\_PATH}{AddPath}{テストプログラム実行時に追加するパス}
	    \Row{PIPE\_PROCESS}{PipeProcess}{%
			...
			実装できていない}
	    \Row{KILL\_PROCESS}{KillProcess}{%
			テストプログラム終了後にkillすべきプログラム\\
			プロセスイメージ名を指定する}
	    \Row{RUN\_LOG}{RunLog}{%
			テスト実行ログファイル名\\
			\Path{\$(SprTest)/log/Run.log}}
	    \Row{RUN\_ERR\_LOG}{RunErrLog}{%
			テスト実行エラーログファイル名\\
...
\begin{Description}{\bf{クラス \tt{RST}}}
	\def\Row#1#2{\tt{#1}&\FCol{294pt}{#2}\\\hline}
	テスト結果累積データのカテゴリを表す定数(enumeration型)を定義するクラス。
	\Vskip{-.8\baselineskip}
...
\begin{Description}{\bf{クラス \tt{CSU}}}
	\def\Row#1#2{\tt{#1}&\FCol{294pt}{#2}\\\hline}
	非公開ソースを使用するか否かを表す定数(enumeration型)を定義するクラス。
	\Vskip{-.8\baselineskip}
	\begin{longtable}[r]{|p{4em}|l|}\hline
	    \MC{1}{|c}{\bf{定 数}} & \MC{1}{|c|}{\bf{説 明}}\\\hline
...
\begin{Description}{\bf{クラス \tt{TESTID}}}
	\def\Row#1#2{\tt{#1}&\FCol{294pt}{#2}\\\hline}
...
\begin{Description}{\bf{クラス \tt{その他の定数}}}
	\def\Row#1#2{\tt{#1}&\FCol{294pt}{#2}\\\hline}
	\Vskip{-.8\baselineskip}
	\begin{longtable}[r]{|p{4em}|l|}\hline
	    \MC{1}{|c}{\bf{定 数}} & \MC{1}{|c|}{\bf{説 明}}\\\hline
