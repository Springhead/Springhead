...
void PHShapePairForLCP::EnumVertex(unsigned ct, PHSolid* solid0, PHSolid* solid1) {
	...
	is1 = shape[0]->FindCutRing(cutRing, shapePoseW[0]);
	...
	if (is0 == SEC_POLYGON && is1 == SEC_POLYGON) {		//	多角形で接触している．
		...
		contactEngine->engine = engine;

		// Handle inactive solids
		if (engine->IsInactiveSolid(solid0->Cast())) contactEngine->SetInactive(1, false);
		if (engine->IsInactiveSolid(solid1->Cast())) contactEngine->SetInactive(0, false);

		// Add the contact engine to the engine's points
		engine->points.push_back(contactEngine);
