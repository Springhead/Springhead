...
public:	
	...
	double pointerDepth;			///< ハプティックポインタの深さ(侵入量)
	Vec3d proxyPos;					///< プロキシの位置(World座標)
	Vec3d lastProxyPos;				///< プロキシの前回の位置(World座標)
	Vec3d objectPos;				///< 接触している相手の物体の現在の位置(World座標)
	Vec3d lastObjectPos;			///< 接触している相手の物体の前回の位置(World座標)
