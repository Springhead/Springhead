...
{
	if ( PyType_Ready( &EPSampleAppType ) < 0 ) return ;//Pythonクラスの作成
	string package;
	if(rootModule) package = PyModule_GetName(rootModule);
	else // rootModuleが渡された場合はEP_MODULE_NAMEは無視される
	{
...
{
	if ( PyType_Ready( &EPHogeshiType ) < 0 ) return ;//Pythonクラスの作成
	string package;
	if(rootModule) package = PyModule_GetName(rootModule);
	else // rootModuleが渡された場合はEP_MODULE_NAMEは無視される
	{
...
{
	if ( PyType_Ready( &EPMyEPAppType ) < 0 ) return ;//Pythonクラスの作成
	string package;
	if(rootModule) package = PyModule_GetName(rootModule);
	else // rootModuleが渡された場合はEP_MODULE_NAMEは無視される
	{
