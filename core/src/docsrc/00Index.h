/** 
\mainpage SpringheadSDK開発者マニュアル
 このマニュアルは，Springhead SDKを開発する方むけのマニュアルです．
 このドキュメントを読む前に，Springhead SDKを利用する方むけのマニュアル 
 <ul>
 <li> <a href="../Springhead.pdf">　<b>ユーザーズマニュアル</b></a>
	&nbsp;使い方の説明 (Springhead Users Manual)
 <li> \ref pageTutorial &nbsp;使い方を学ぶための練習コース
 <li> <a href="../Reference/index.html">　<b>リファレンスマニュアル</b></a>
	&nbsp;APIの詳細な説明<br>
 </ul>
 を先にご覧ください．
 Springhead2を使用していて，APIに不足を感じたり，SDKの拡張の必要があると
 感じた場合，SDKを開発する場合には，こちらをお読みください．
<br>

Springhead2SDK開発者マニュアルは，
 - \subpage pageAPIClassImp
 - \subpage pageFoundationImp
 - \subpage pagePhysicsImp
 - \subpage pageGraphicsImp
 - \subpage pageFileIOImp
 - \subpage pageSDKNote

から構成されます．

*/
