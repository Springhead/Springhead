...
namespace SprCs {
    ...
    public partial class PHBodyIf : SceneObjectIf {
        ...
        public Posed GetPose() {
            ...
            if (phSceneIf.multiThreadMode) {
                ...
                if (currentThread == phSceneIf.stepThread) {
                    ...
                    (IntPtr)_thisArray[phSceneIf.sceneForStep]); // ‚±‚±‚ÅŽæ“¾‚³‚ê‚éPosed‚Í•¡»
                    return new Posed(ptr, true);
                } else if (currentThread == phSceneIf.subThread) {
                    lock (phSceneIf.phSceneLock) {
                        phSceneIf.isSubThreadRunning = true;
                        IntPtr ptr = SprExport.Spr_PHBodyIf_GetPose(
        ...
        public void AddShape(CDShapeIf shape) {
            ...
            if (phSceneIf.multiThreadMode) {
                lock (phSceneIf.phSceneLock) {
                    if (phSceneIf.isStepThreadRunning) {
    ...
    public partial class PHBallJointIf : PHJointIf {
        ...
        public void SetOffsetForce(Vec3d ofst) {
            ...
            if (phSceneIf.multiThreadMode) {
                var currentThread = Thread.CurrentThread;
                if (currentThread == phSceneIf.stepThread) {
                    lock (phSceneIf.phSceneLock) { // callbackForStepThread‚ðsubThread‚ÅŽQÆ‚·‚é‚Ì‚Å•K—v‚Èlock
        ...
        public PHSolidIf CreateSolid(PHSolidDesc desc) {
            if (multiThreadMode) {
                lock (phSceneLock) {
                    ...
                    if (stateForSwap != null) { // Create‚ðƒƒ‚ƒŠƒŠ[ƒN‚³‚¹‚È‚¢‚½‚ß‚ÉCˆê”ÔÅ‰‚ÌCreate‚ð‚µ‚È‚¢‚½‚ßCˆê”ÔÅ‰‚¾‚¯null‚É‚È‚é
        ...
        public PHJointIf CreateJoint(PHSolidIf lhs, PHSolidIf rhs, IfInfo ii, PHJointDesc desc) {
            if (multiThreadMode) {
                ...
                lock (phSceneLock) {
                    ...
                    if (stateForSwap != null) { // Create‚ðƒƒ‚ƒŠƒŠ[ƒN‚³‚¹‚È‚¢‚½‚ß‚ÉCˆê”ÔÅ‰‚ÌCreate‚ð‚µ‚È‚¢‚½‚ßCˆê”ÔÅ‰‚¾‚¯null‚É‚È‚é
                        SprExport.Spr_ObjectStatesIf_ReleaseState(stateForSwap._this, _thisArray[sceneForGet]); // ‹°‚ç‚­stateForSwap._this‚Æ_thisArray[sceneForGet]‚ª“¯‚¶\‘¢‚Å‚È‚¢‚Æ‚¢‚¯‚È‚¢‚Ì‚Å‚±‚±‚Å
                    }
                    callObjectStatesIf_Create = true;
                    if (isStepThreadRunning) {
            ...
            } else { // ƒRƒ“ƒXƒgƒ‰ƒNƒ^‚ÌŽÀ‘•‚É‚æ‚Á‚ÄC³‚ª•K—v
            ...
            if (multiThreadMode) {
                ...
                if (currentThread == stepThread) {
                    int result = SprExport.Spr_PHSceneIf_NContacts(
                        (IntPtr)_thisArray[sceneForStep]); // ‚±‚±‚ÅŽæ“¾‚³‚ê‚éPosed‚Í•¡»
                    return result;
                } else if (currentThread == subThread) {
                    lock (phSceneLock) {
                        isSubThreadRunning = true;
                        int result = SprExport.Spr_PHSceneIf_NContacts(
                            (IntPtr)_thisArray[sceneForGet]); // ‚±‚±‚ÅŽæ“¾‚³‚ê‚éPosed‚Í•¡»
                        return result;
                    }
                }
            } else {
            ...
            if (multiThreadMode) {
                ...
                if (currentThread == stepThread) {
                    IntPtr ptr = SprExport.Spr_PHSceneIf_GetContact(
                        (IntPtr)_thisArray[sceneForStep], (int)i); // ‚±‚±‚ÅŽæ“¾‚³‚ê‚éPosed‚Í•¡»
                    if (ptr == IntPtr.Zero) { return null; }
                    ...
                    lock (phSceneLock) {
                        isSubThreadRunning = true;
                        IntPtr ptr = SprExport.Spr_PHSceneIf_GetContact(
                            (IntPtr)_thisArray[sceneForGet], (int)i); // ‚±‚±‚ÅŽæ“¾‚³‚ê‚éPosed‚Í•¡»
                        if (ptr == IntPtr.Zero) { return null; }
                        PHContactPointIf obj = new PHContactPointIf(ptr);
                ...
                IntPtr ptr = SprExport.Spr_PHSceneIf_GetContact(
                    (IntPtr)_thisArray[0], (int)i); // ‚±‚±‚ÅŽæ“¾‚³‚ê‚éPosed‚Í•¡»
                if (ptr == IntPtr.Zero) { return null; }
                PHContactPointIf obj = new PHContactPointIf(ptr);
                return obj;
            }
