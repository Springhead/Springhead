#define SPR_DECLMEMBEROF_CDShapePairState \
protected:\
	Vec3d	normal;	\
	double	depth;	\
	unsigned int	lastContactCount;	\
	unsigned int	contactDuration;	\
public:\
	virtual void SetState(const void* ptr){ \
		normal = ((CDShapePairState*)ptr)->normal;	\
		depth = ((CDShapePairState*)ptr)->depth;	\
		lastContactCount = ((CDShapePairState*)ptr)->lastContactCount;	\
		contactDuration = ((CDShapePairState*)ptr)->contactDuration;	\
	}\
	virtual bool GetState(void* ptr) const { \
		((CDShapePairState*)ptr)->normal = normal;	\
		((CDShapePairState*)ptr)->depth = depth;	\
		((CDShapePairState*)ptr)->lastContactCount = lastContactCount;	\
		((CDShapePairState*)ptr)->contactDuration = contactDuration;	\
		return true;	\
	}\
...
#define SPR_DECLMEMBEROF_CDEllipsoidDesc \
protected:\
	Vec3d	radius;	\
public:\
	virtual const void* GetDescAddress() const { return NULL; }\
	virtual void SetDesc(const void* ptr){ \
		CDShape::SetDesc((CDShapeDesc*)(CDEllipsoidDesc*)ptr);	\
		radius = ((CDEllipsoidDesc*)ptr)->radius;	\
		AfterSetDesc();	\
	}\
	virtual bool GetDesc(void* ptr) const { \
		BeforeGetDesc();	\
		CDShape::GetDesc((CDShapeDesc*)(CDEllipsoidDesc*)ptr);	\
		((CDEllipsoidDesc*)ptr)->radius = radius;	\
		return true;	\
	}\
