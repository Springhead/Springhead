...
class VisualStudio:
	...
	def __get_vs_path(self, version):
		...
		if self.vs_version <= '14.0':
			...
			sys_base = 'C:/Program Files (x86)'
			usr_base = 'C:%s/Application' % Util.upath(os.environ['HOMEPATH'])
			for base in [usr_base, sys_base]:
				devenvpath = base \
					   + '/Microsoft Visual Studio' \
					   + '/%s' % self.vs_name.split()[2] \
					   + '/Community/Common7/IDE'
				#print('-- TRY: %s' % devenvpath)
				if os.path.exists('%s/%s' % (devenvpath, devenv)):
					break
		if self.verbose:
			print('  devenv path: %s' % devenvpath)
		if not os.path.exists(Util.pathconv('%s/%s' % (devenvpath, devenv))):
