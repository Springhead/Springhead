#include "SliderChain.h"
#include "SampleModel.h"

#define ESC 27

SliderChain::SliderChain(){
	bDrawInfo = false;
}

void SliderChain::Init(int argc, char* argv[]){
	CreateSdk();											// Sdkの作成
...
void SliderChain::TimerFunc(int id){	
	GetSdk()->Step();
	PostRedisplay();
}

void SliderChain::InitCameraView(){
...
void SliderChain::CreateObjects(){
...
void SliderChain::Reset(){
	GetSdk()->GetScene()->GetPHScene()->Clear();
	CreateObjects();
}

void SliderChain::Keyboard(int key, int x, int y){
	switch (key) {
		case ESC:
		case 'q':
			FWApp::Clear();
