...
void SliderTest::Init(int argc, char* argv[]){
	CreateSdk();											// Sdkの作成
...
void SliderTest::TimerFunc(int id){	
	GetSdk()->Step();
	PostRedisplay();
}

void SliderTest::InitCameraView(){
...
void SliderTest::Reset(){
	GetSdk()->GetScene()->GetPHScene()->Clear();
	CreateObjects();
}

void SliderTest::Keyboard(int key, int x, int y){
	switch (key) {
		case ESC:
		case 'q':
			FWApp::Clear();
