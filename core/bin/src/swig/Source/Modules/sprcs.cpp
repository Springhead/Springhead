...
#define	TRY_CASE_RPTR	2
			...
			if (GetFlagAttr(members[i], "feature:only_cs_ignore") == NULL) {
				...
				is_virtual = GetFlagAttr(members[i], "feature:virtual_function");
				if (ni.is_vector || ni.is_array) {
					char* wrapper_class = make_wrapper_name(fps, FD_CS, __LINE__, ni, ci, "function_return_type");
					Printf(CS, "\tpublic %s%s%s", (is_virtual ? "virtual ": ""), (ni.is_static ? "static" : ""), wrapper_class);
					...
					Printf(CS, "\tpublic %s%s%s", (is_virtual ? "virtual ": ""), (ni.is_static ? "static " : ""), ni.cs_type);
				}
				...
				bool is_change_objectstate_function = false; // ObjectStateのメモリ構成が変更されるメソッド

				bool is_all_elements_thisArray_method = false; // 全ての_thisArrayでメソッドを実行する
				bool is_phsceneobject = false; // phSceneに含まれるオブジェクト
				bool is_return_thisArray0 = true; // _thisArray[0]に設定してリターン
				...
				Node* return_cs_type_node = FindNodeByAttrR(topnode, "name", ni.type);
				if (return_cs_type_node != NULL) {
					is_return_thisArray0 = !IsUseAll_thisArray(return_cs_type_node, topnode);
				}

				if (is_all_elements_thisArray_class && !ni.is_static) {
					if (((string(ni.cs_name).find("Get") == 0 || // 先頭がGet,Is
						string(ni.cs_name).find("Is") == 0 || 
						(string(ni.cs_name).find("N") == 0 && string(ni.cs_name).length() > 2 && isupper(string(ni.cs_name)[1]))) // Nが先頭且つ二文字目が大文字
						/*&& is_return_thisArray0*/) || // Ifクラスでない
						GetFlagAttr(members[i], "feature:only_thisArray0_method") // .iファイルで設定されている場合
						) {
						is_all_elements_thisArray_method = false;
					}
					else if (string(ni.cs_name).find("Create") == 0) { // Create系はほとんど
						is_change_objectstate_function = true;
						is_all_elements_thisArray_method = true;
					}
					else {
						is_all_elements_thisArray_method = true;
					}
				...
				if (!ni.is_static && is_all_elements_thisArray_class && !is_return_thisArray0) {
					Printf(CS, "// is_all_elements_thisArray_class is_return_thisArray0\n");
				}
				...
				if (!ni.is_static && is_all_elements_thisArray_class && !is_all_elements_thisArray_method) { // GetPoseなど
					...
					CSFunctionReturnCode(ni, ci, topnode, "Step", false, is_return_thisArray0 ? "0" : "phSceneIf.sceneForStep", false, true, fps);
					...
					CSFunctionReturnCode(ni, ci, topnode, "Get", false, is_return_thisArray0 ? "0" : "phSceneIf.sceneForGet", false, false, fps);
					Printf(CS, "				}\n");
					Printf(CS, "			}\n");
					Printf(CS, "		} else {\n");
					...
					CSFunctionReturnCode(ni, ci, topnode, "", false, "0", false, false, fps);
					Printf(CS, "		}\n");
					...
					csfunction_code(/* multithread = */false, /* lvalue_name = */"Step", /* string_index = */ 0, "_thisArray[phSceneIf.sceneForStep]", fps, topnode, n, ni, callback_argnames, cleanup1, cleanup2, is_enum_node, ci, sep_needed, class_already_defined);
					...
					if (is_change_objectstate_function) {
						Printf(CS, "					if (phSceneIf.stateForSwap != null) {\n");
						Printf(CS, "						SprExport.Spr_ObjectStatesIf_ReleaseState(phSceneIf.stateForSwap._thisArray[0], _thisArray[phSceneIf.sceneForGet]);\n");
						Printf(CS, "					}\n");
						Printf(CS, "					phSceneIf.callObjectStatesIf_Create = true;\n");
					}
					...
					CSFunctionReturnCode(ni, ci, topnode, "Get", false, is_return_thisArray0 ? "0" : "phSceneIf.sceneForGet", true, false, fps, /* multithread = */false, /* lvalue_name = */"Step", /* string_index = */ string_index, "_thisArray[phSceneIf.sceneForStep]", n, callback_argnames, cleanup1, cleanup2, sep_needed, class_already_defined);
					...
					CSFunctionReturnCode(ni, ci, topnode, "Get", !is_return_thisArray0, is_return_thisArray0 ? "0" : "phSceneIf.sceneForGet", false, false, fps);
					//Printf(CS, "						}\n");
					Printf(CS, "					}\n");
					Printf(CS, "				}\n");
					Printf(CS, "			}\n");
	...
	void csfunction_code(bool multithread, char* lvalue_name, int string_index, char* instance_name, DOHFile* fps[], Node* topnode, Node* n, NodeInfo& ni, char** argnames, void** cleanup1, void** cleanup2, Node* is_enum_node, NodeInfo& ci, int sep_needed, bool class_already_defined = false) {			
			// 引数に関する前処理
			...
			for (int j = 0; j < ni.num_args; j++) {
				...
				if (ai.is_string) {
					...
					//}
					//else {
						sprintf(tmpbuff, "remote%s%d", lvalue_name, j+1);
					//}
			...
			is_enum_node = (ni.is_struct) ? FindNodeByAttrR(topnode, "enumtype", ni.type) : NULL;
			if (ni.is_bool) {
				Printf(CS, "\t    char ret%s = ", lvalue_name);
			}
			else if (ni.is_string) {
				Printf(CS, "\t    IntPtr ptr%s = ", lvalue_name);
			}
			else if (ni.is_vector || ni.is_array) {
				Printf(CS, "\t    IntPtr ptr%s = ", lvalue_name);
			}
				...
				if (is_enum_node) {
					Printf(CS, "\t    int result%s = ", lvalue_name);
				} else {
					Printf(CS, "\t    IntPtr ptr%s = ", lvalue_name);
				}
			}
				...
				Printf(CS, "\t    %s result%s = (%s) ", ni.cs_type, lvalue_name, ni.cs_type);
			}
			else if (!ni.is_void) {
				Printf(CS, "\t    %s result%s = (%s)(Object) ", ni.cs_type, lvalue_name, ni.cs_type);
			}
			else {
				Printf(CS, "\t    ");
			}
			...
			sep_needed = 0;
			if (!ni.is_static) {
				Printf(CS, "(IntPtr) %s", instance_name);
				sep_needed = 1;
			}
			for (int j = 0; j < ni.num_args; j++) {
				NodeInfo& ai = ni.funcargs[j];
	...
	// 戻り値のための処理 引数isStepThreadExecutingPart以降はcsfunction_codeの引数のためisStepThreadExecuting
	void CSFunctionReturnCode(NodeInfo& ni, NodeInfo& ci, Node* topnode, char* lvalue_name, bool ifclass_all_thisArray, char* instance_index, bool isStepThreadExecutingPart, bool isStepThreadPart, DOHFile* fps[], bool multithread = false, char* lvalue_name_csfunction = NULL,int string_index = 0, char* instance_name = NULL, Node* n = NULL, char** argnames = NULL, void** cleanup1 = NULL, void** cleanup2 = NULL, int sep_needed = 0, bool class_already_defined = false) {
		Node* is_enum_node = (ni.is_struct) ? FindNodeByAttrR(topnode, "enumtype", ni.type) : NULL;
	  	// 関数戻り値のための後処理
	  	if (ni.is_bool) {
	  		SNAP_ANA_PATH1(fps, FD_CS, "function_return: intrinsic: bool");
	  		Printf(CS, "\t    return (ret%s == 0) ? false : true;\n", lvalue_name);
	  	}
	  	else if (ni.is_pointer && EQ(ni.uq_type, "char")) {
	  		Printf(CS, "\t    return Marshal.PtrToStringAnsi(ptr%s);\n", lvalue_name);
	  	}
	  	else if (ni.is_string) {
	  		SNAP_ANA_PATH1(fps, FD_CS, "function_return: string");
	    	Printf(CS, "            string bstr = Marshal.PtrToStringBSTR(ptr%s);\n", lvalue_name);
#if (FREE_UNMANAGED_MEMORY == 1)
			Printf(CS, "            SprExport.Spr_%s_FreeString_%s(ptr%s);\n", ci.uq_name, ni.uq_name, lvalue_name);
#endif
			...
			Printf(CS, "            return new %s(ptr%s);\n", wrapper_name, lvalue_name);
		}
		else if (ni.is_struct) {
			if (is_enum_node) {
				SNAP_ANA_PATH1(fps, FD_CS, "function_return: enum");
				Printf(CS, "            return (%s) result%s;\n", cs_qualified_name(unqualified_name(ni.type)), lvalue_name);
			}
			else if (ENDWITH(ni.cs_type, "If")) {
				Printf(CS, "            if (ptr%s == IntPtr.Zero) { return null; } \n", lvalue_name, ni.cs_type, ni.cs_type);
				if (ifclass_all_thisArray) {
					Printf(CS, "            %s obj = new %s(ptrStep, ptrBuffer, ptrGet, phSceneIf.sceneForStep, phSceneIf.sceneForBuffer, phSceneIf.sceneForGet);\n", ni.cs_type, ni.cs_type, lvalue_name, instance_index);
				}
				else {
					if (isStepThreadExecutingPart) {
						Printf(CS, "            %s obj = new %s(ptrGet ,ptrBuffer ,phSceneIf.sceneForGet ,phSceneIf.sceneForBuffer);\n", ni.cs_type, ni.cs_type);
					}
					else {
				...
				if (isStepThreadExecutingPart && childClassMap[ni.cs_type].size() > 0) {
					Printf(CS, "			} else { \n");
					Printf(CS, "				phSceneIf.AddCallbackForSubThread(() => {\n");
					csfunction_code(/* multithread = */false, /* lvalue_name = */lvalue_name_csfunction, /* string_index = */ string_index, instance_name, fps, topnode, n, ni, argnames, cleanup1, cleanup2, is_enum_node, ci, sep_needed, class_already_defined);
					Printf(CS, "					obj.%s = ptr%s;\n", instance_name, lvalue_name_csfunction);
					Printf(CS, "				});\n");
					if (childClassMap[ni.cs_type].size() > 0) {
						Printf(CS, "			}\n");
					}
				}
				else if(childClassMap[ni.cs_type].size() > 0){
					Printf(CS, "            }\n");
				}
	...
	bool IsUseAll_thisArray(Node* n, Node *topnode) {
		if (GetFlagAttr(n, "feature:only_thisArray0_class")) {
			return false;
		}
...
***/
	...
	void generate_constructor(DOHFile* fps[], Node* node, char* name, char* uq_name, char* cpp_name, StructInfo* struct_info, int newable = 1) {
		...
		if (newable) {
			...
			Printf(CS, "\tpublic %s(IntPtr ptr0, IntPtr ptr1, int ptr0Index, int ptr1Index) : base(ptr0, ptr1, ptr0Index, ptr1Index, false) {}\n", name);
			Printf(CS, "\tpublic %s(IntPtr ptr0, IntPtr ptr1, int ptr0Index, int ptr1Index, bool flag) : base(ptr0, ptr1, ptr0Index, ptr1Index, flag) {}\n", name);
			Printf(CS, "\tpublic %s(IntPtr ptr0, IntPtr ptr1, IntPtr ptr2, int ptr0Index, int ptr1Index, int ptr2Index) : base(ptr0, ptr1, ptr2, ptr0Index, ptr1Index, ptr2Index, false) {}\n", name);
			Printf(CS, "\tpublic %s(IntPtr ptr0, IntPtr ptr1, IntPtr ptr2, int ptr0Index, int ptr1Index, int ptr2Index, bool flag) : base(ptr0, ptr1, ptr2, ptr0Index, ptr1Index, ptr2Index, flag) {}\n", name);
#else
