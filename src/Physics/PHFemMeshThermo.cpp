...
PHFemMeshThermo::PHFemMeshThermo(const PHFemMeshThermoDesc& desc, SceneIf* s){
	...
	temp_c = 30.0;
	temp_out = 23.8;	//A5052試料時室温:23.8, SUS430放熱実験時:29.9
...
void PHFemMeshThermo::SetWeekPow(double weekPow2){
	weekPow_ = weekPow2;
}

void PHFemMeshThermo::SetOuterTemp(double temp){
	jout = temp;
}
void PHFemMeshThermo::SetThermalRadiation(double ems_,double ems_const_){
	ems = ems_;
	ems_const = ems_const_;
}
void PHFemMeshThermo::SetGaussCalcParam(unsigned cyc,double epsilon_){
	NofCyc = cyc;
	epsilonG = epsilon_;
}
...
void PHFemMeshThermo::CalcVtxDisFromVertex(Vec2d originVertexIH){
	...
	for(unsigned i=0;i<nSurfaceFace;i++){
		...
		if(vertices[faces[i].vertices[0]].pos.y < 0.0 && vertices[faces[i].vertices[1]].pos.y < 0.0 && vertices[faces[i].vertices[2]].pos.y < 0.0){
			...
			//	(x,z)平面におけるmayIHheatedのface全節点の原点からの距離を計算する
			for(unsigned j=0; j<3; j++){
				double dx = vertices[faces[i].vertices[j]].pos.x - originVertexIH[0];
				double dz = vertices[faces[i].vertices[j]].pos.z - originVertexIH[1];	//	表記はyだが、実質z座標が入っている
...
//		DSTR << "dqdtds:  " << dqdtds << std::endl;
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].fluxarea){
				faces[i].heatflux[num][0] = faces[i].fluxarea[0] / faceS * dqdtAll;		//	熱流束の量をheatfluxの面積から計算
//				DSTR << "faces[" << i <<"].heatflux: " << faces[i].heatflux <<std::endl;			
				// debug
				//for(unsigned j = 0; j < 3;++j){
...
void PHFemMeshThermo::CalcIHdqdtband(double xS,double xE,double dqdtAll,unsigned num){
	...
	for(unsigned i=0;i < nSurfaceFace; i++){
		if(faces[i].mayIHheated){			// faceの節点のy座標が負の場合→IH加熱の対象節点
			...
			if( nObinnerVtx == 1)			faces[i].fluxarea[0] = faces[i].area;//faces[i].fluxarea = 1.0/3.0 * faces[i].area;
			else if(nObinnerVtx == 2)		faces[i].fluxarea[0] = faces[i].area;//faces[i].fluxarea = 2.0/3.0 * faces[i].area;
			else if(nObinnerVtx == 3)		faces[i].fluxarea[0] = faces[i].area;
			else if(nObinnerVtx == 0)		faces[i].fluxarea[0] = 0.0;
...
//		DSTR << "dqdtds:  " << dqdtds << std::endl;
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				faces[i].heatflux[num][0] = dqdtds * faces[i].fluxarea[0];		//	熱流束の量をheatfluxの面積から計算
//				DSTR << "faces[" << i <<"].heatflux: " << faces[i].heatflux <<std::endl;
			}
		}
	}
...
void PHFemMeshThermo::CalcIHdqdt_decrease_high(double r,double R,double dqdtAll,unsigned mode){
	//dqdtAllを単位面積辺り位に直す
	...
	for(unsigned i=0;i < nSurfaceFace; i++){
		if(faces[i].mayIHheated){			// faceの節点のy座標が負の場合→IH加熱の対象節点
			...
			for(unsigned j=0;j<3;j++){
				if( r <= vertices[faces[i].vertices[j]].disFromOrigin && vertices[faces[i].vertices[j]].disFromOrigin <= R){
					faces[i].fluxarea[2] = faces[i].area;
					break;
				}
			}
			faceS += faces[i].fluxarea[2];
			cnt+=1;
		}
	}
	double debugS=0.0;
	...
	if(faceS > 0){
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				double avgTemp=0.0;
				double ohm = 0.0;
				avgTemp = 1.0 / 3.0 * ( vertices[faces[i].vertices[0]].temp + vertices[faces[i].vertices[1]].temp + vertices[faces[i].vertices[2]].temp);
				...
				if(avgTemp > 25.0){
					faces[i].heatflux[mode][0] = dqdtds * (5.86e-7)/(ohm);		//	熱流束の量をheatfluxの面積から計算し、J/m^2に直さなければいけない→行列で計算するために。
				}else{
					faces[i].heatflux[mode][0] = dqdtds;
				}
				//debug
				debugdq += dqdtds * faces[i].fluxarea[2];
				debugS += faces[i].fluxarea[2];
			}
		} 
	}
...
void PHFemMeshThermo::CalcIHdqdt_decrease(double r,double R,double dqdtAll,unsigned mode){
	...
	if(faceS > 0){
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				faces[i].heatflux[mode][2] = -dqdtds;		//	熱流束の量をheatfluxの面積から計算し、J/m^2に直さなければいけない→行列で計算するために。
				//debug
				debugdq += dqdtds * faces[i].fluxarea[2];
				debugS += faces[i].fluxarea[2];
			}
...
void PHFemMeshThermo::CalcIHdqdt_add_high(double r,double R,double dqdtAll,unsigned mode){
	//%%%			[1]について計算
	//dqdtAllを単位面積辺り位に直す
	double dqdtdsAll =0.0;
	...
	for(unsigned i=0;i < nSurfaceFace; i++){
		if(faces[i].mayIHheated){			// faceの節点のy座標が負の場合→IH加熱の対象節点
			...
			for(unsigned j=0;j<3;j++){
				if( r <= vertices[faces[i].vertices[j]].disFromOrigin && vertices[faces[i].vertices[j]].disFromOrigin <= R){
					faces[i].fluxarea[1] = faces[i].area;
					break;
				}
			}
			faceS += faces[i].fluxarea[1];
			cnt+=1;
		}
	}
	double debugS=0.0;
	...
	if(faceS > 0){
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				...
				//debug
				debugdq += dqdtds * faces[i].fluxarea[1];
				debugS += faces[i].fluxarea[1];
			}
		} 
	}
	if(debugS != faceS){ DSTR << "diff between debugS:" << debugS << ", faceS:" << faceS << std::endl; }
...
void PHFemMeshThermo::CalcIHdqdt_add(double r,double R,double dqdtAll,unsigned mode){
	...
	if(faceS > 0){
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				faces[i].heatflux[mode][1] = dqdtds;		//	熱流束の量をheatfluxの面積から計算し、J/m^2に直さなければいけない→行列で計算するために。
				//debug
				debugdq += dqdtds * faces[i].fluxarea[1];
				debugS += faces[i].fluxarea[1];
			}
	...
	if(faceS > 0){
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				...
				//debug
				debugdq += dqdtds * faces[i].fluxarea[0];
				debugS += faces[i].fluxarea[0];
			}
		} 
	}
	if(debugS != faceS){ DSTR << "diff between debugS:" << debugS << ", faceS:" << faceS << std::endl; }
...
void PHFemMeshThermo::CalcIHdqdt_atleast(double r,double R,double dqdtAll,unsigned mode){
	...
	if(faceS > 0){
		...
		for(unsigned i=0;i < nSurfaceFace; i++){
			if(faces[i].mayIHheated){
				faces[i].heatflux[mode][0] = dqdtds;		//	熱流束の量をheatfluxの面積から計算し、J/m^2に直さなければいけない→行列で計算するために。
				//debug
				debugdq += dqdtds * faces[i].fluxarea[0];
				debugS += faces[i].fluxarea[0];
			}
...
void PHFemMeshThermo::CalcIHdqdt(double r,double R,double dqdtAll,unsigned num){
	...
	for(unsigned i=0;i < nSurfaceFace; i++){
		if(faces[i].mayIHheated){			// faceの節点のy座標が負の場合→IH加熱の対象節点
			...
			if( nObinnerVtx == 1)			faces[i].fluxarea[0] = 1.5/3.0 * faces[i].area;//faces[i].fluxarea = 1.0/3.0 * faces[i].area;
			else if(nObinnerVtx == 2)		faces[i].fluxarea[0] = 2.8/3.0 * faces[i].area;//faces[i].fluxarea = 2.0/3.0 * faces[i].area;
			else if(nObinnerVtx == 3)		faces[i].fluxarea[0] = faces[i].area;
...
void PHFemMeshThermo::CalcHeatTransDirect2(double dt){
	//lapack利用
...
//	DSTR << "tets[0].volume: " << tets[0].volume << std::endl;
	...
	TVecAll2.resize(vertices.size());
	TVecAll2 = TVecAll;


	PTM::VVector<double> TVecAll3;	
	// define @.h
	TVecAll3.resize(vertices.size());
	TVecAll3 = TVecAll;
...
void PHFemMeshThermo::UpdateVertexTempAll(){
	for(unsigned i=0;i < vertices.size();i++){
...
void PHFemMeshThermo::InitToutAll(double temp){
	for(unsigned i =0; i < vertices.size();i++){
		vertices[i].Tout = temp;
...
void PHFemMeshThermo::InitCreateMatC(){
	...
	matCAll.resize(vertices.size(),vertices.size());
	matCAll.clear();
}
...
//void PHFemMeshThermo::UpdateMatK(){
//
//	//%%%	初期化類		%%%//
...
void PHFemMeshThermo::IfRadiantHeatTrans(){
	for(int id =0;id<NSurfaceVertices();id++){
		if(vertices[id].beRadiantHeat){
			...
			vertices[id].thermalEmissivity = 0.0;
			vertices[id].thermalEmissivity_const = 0.0;
		}	
	}
}

void PHFemMeshThermo::UpdateMatk_RadiantHeatToAir(){
		for(unsigned i =0; i < edges.size();i++){
			edges[i].k = 0.0;
		}
#ifdef UseMatAll
...
//void PHFemMeshThermo::ReProduceMat_Vec_ThermalRadiation() {	
//
//	//%%%	初期化類		%%%//
//
//	//各種メンバ変数の初期化⇒コンストラクタでできたほうがいいかもしれない。
...
#if 0		// 値は入っていた
	//	値の確認
	DSTR << "熱放射率," << "Tout," <<  "j以遠温度," << std::endl;
	DSTR << ems <<"," << vertices[0].Tout <<"," << jout << std::endl;
	DSTR << vertices[0].Tc <<"," << vertices[0].Tout <<"," << vertices[0].heatTransRatio<< "," << vertices[0].thermalEmissivity << std::endl;
	DSTR << temp_c << "," << temp_out << std::endl;
	DSTR << weekPow_  << ","<< inr_<< "," << outR_<< "," << stopTime<< "," << radiantHeat << std::endl;
#endif
	//vertices.tempをすべて、TVecAllへ代入する
	CreateTempVertex();
...
void PHFemMeshThermo::OutputMatKall(){
	matKAllout.open("matKAllout.txt");
	matKAllout << matKAll << std::endl;
}
...
void PHFemMeshThermo::CreatedMatCAll(unsigned id){
	...
	for(unsigned j=0; j<4 ; j++){
		for(unsigned k=0; k<4 ;k++){
			matCAll[tets[id].vertices[j]][tets[id].vertices[k]] += matc[j][k];
		}
	}
#endif UseMatAll
...
void PHFemMeshThermo::CreateVecFAll(unsigned id){
	...
	vecf += tets[id].vecf[1] + tets[id].vecf[3];
#endif
...
void PHFemMeshThermo::CreateMatkLocal_update(unsigned id){
...
//	tets[id].matk[0]= thConduct / (36 *  CalcTetrahedraVolume(tets[id])) * tets[id].matk[0];		//理論が間違っていたので、修正
	tets[id].matk[0]= thConduct / (36 *  tets[id].volume) * tets[id].matk[0];		//理論が間違っていたので、修正
...
//	matk1 = thConduct / (36 * CalcTetrahedraVolume(tets) ) * matk1;
	...
	tets[id].matk[0] =  1.0 / ( 36.0 * tets[id].volume ) * tets[id].matk[0];
...
PTM::TMatrixRow<4,4,double> PHFemMeshThermo::GetKMatInTet(unsigned id){
	//for(unsigned id=0;id<tets.size();++id){
	return tets[id].matk[0];
	//}
}
		...
		if(tets[id].faces[l] < (int)nSurfaceFace && faces[tets[id].faces[l]].fluxarea > 0 ){			///	外殻の面 且つ 熱伝達率が更新されたら matk2を更新する必要がある
			...
			for(unsigned j=0;j<4;j++){
				if(tets[id].vertices[j] == ID){					///	形状関数が１、（すなわち）このfaceに対面する頂点　と一致したら　その時のfaceで面積分する
					...
					for(unsigned i =0;i<3;++i){
						tets[id].vecf[1] += faces[tets[id].faces[l]].heatflux[num][i] * (1.0/3.0) * faces[tets[id].faces[l]].area * vecf2array[j];
					}
...
void PHFemMeshThermo::CreateVecf3surface(unsigned id){
	// 初期化
	tets[id].vecf[2].clear();
	double Tc[4];
	double TcAll =0;
	for(unsigned l=0;l<4;++l){
		TcAll += vertices[tets[id].vertices[l]].Tc;
	}
	//l=0の時f21,1の時:f22, 2の時:f23, 3の時:f24	を生成
	///	..j番目の行列の成分が0のベクトルを作成
	for(unsigned l= 0 ; l < 4; l++){
		...
		Tc[l] =  (TcAll - vertices[tets[id].vertices[l]].Tc) * 1.0/3.0;
	}
	for(unsigned l= 0 ; l < 4; l++){
		///	四面体の各面(l = 0 ～ 3) についてメッシュ表面かどうかをチェックする。表面なら、行列を作ってvecf2arrayに入れる
		//faces[tets.faces[i]].sorted;		/// 1,24,58みたいな節点番号が入っている
		if(tets[id].faces[l] < (int)nSurfaceFace && faces[tets[id].faces[l]].heatTransRatios.size() ){		///	外殻の面でのみ実行 && （１つ以上の）熱伝達率が設定されているとき
			///	四面体の三角形の面積を計算		///	この関数の外で面積分の面積計算を実装する。移動する
			if(faces[tets[id].faces[l]].area ==0 || faces[tets[id].faces[l]].deformed ){		///	面積が計算されていない時（はじめ） or deformed(変形した時・初期状態)がtrueの時		///	条件の追加	面積が0か ||(OR) αが更新されたか
				faces[tets[id].faces[l]].area = CalcTriangleArea(faces[tets[id].faces[l]].vertices[0], faces[tets[id].faces[l]].vertices[1], faces[tets[id].faces[l]].vertices[2]);
				faces[tets[id].faces[l]].deformed = false;
			...
			for(unsigned j=0;j<4;j++){
				if(tets[id].vertices[j] == ID){					///	形状関数が１、（すなわち）このfaceに対面する頂点　と一致したら　その時のfaceで面積分する
					...
					for(unsigned k =0 ; k < faces[tets[id].faces[l]].heatTransRatios.size(); ++k){
						tets[id].vecf[2] +=  faces[tets[id].faces[l]].heatTransRatios[k] * (1.0/3.0) * faces[tets[id].faces[l]].area * vecf3array[j];	//+=:j=0~3のvecf2arrayを加算			
					}
				}
			}
		}
...
void PHFemMeshThermo::CreateVecf4surface(unsigned id){
	// 初期化
	tets[id].vecf[3].clear();
	//l=0の時f21,1の時:f22, 2の時:f23, 3の時:f24	を生成
	///	..j番目の行列の成分が0のベクトルを作成
	double Tout[4];
	double ToutAll =0;
	for(unsigned l=0;l<4;++l){
		ToutAll += vertices[tets[id].vertices[l]].Tout;
	}
	for(unsigned l= 0 ; l < 4; l++){
		vecf4array[l] = Create41Vec1();
		vecf4array[l][l] = 0.0;			//	l行を0に
		Tout[l] =  (ToutAll - vertices[tets[id].vertices[l]].Tout) * 1.0/3.0;
	}
	for(unsigned l= 0 ; l < 4; l++){
		///	四面体の各面(l = 0 ～ 3) についてメッシュ表面かどうかをチェックする。表面なら、行列を作ってvecf2arrayに入れる
		//faces[tets.faces[i]].sorted;		/// 1,24,58みたいな節点番号が入っている
		if(tets[id].faces[l] < (int)nSurfaceFace){	// && faces[tets[id].faces[l]].thermalEmissivity != 0 ){		///	外殻の面でのみ実行 && （１つ以上の）熱伝達率が設定されているとき
			///	四面体の三角形の面積を計算		///	この関数の外で面積分の面積計算を実装する。移動する
			if(faces[tets[id].faces[l]].area ==0 || faces[tets[id].faces[l]].deformed ){		///	面積が計算されていない時（はじめ） or deformed(変形した時・初期状態)がtrueの時		///	条件の追加	面積が0か ||(OR) αが更新されたか
				faces[tets[id].faces[l]].area = CalcTriangleArea(faces[tets[id].faces[l]].vertices[0], faces[tets[id].faces[l]].vertices[1], faces[tets[id].faces[l]].vertices[2]);
				faces[tets[id].faces[l]].deformed = false;
			...
			for(unsigned j=0;j<4;j++){
				if(tets[id].vertices[j] == ID){					///	形状関数が１、（すなわち）このfaceに対面する頂点　と一致したら　その時のfaceで面積分する
					double ems_temp = ( 	
						vertices[faces[tets[id].faces[l]].vertices[0]].thermalEmissivity * vertices[faces[tets[id].faces[l]].vertices[0]].temp + vertices[faces[tets[id].faces[l]].vertices[0]].thermalEmissivity_const
						+ vertices[faces[tets[id].faces[l]].vertices[1]].thermalEmissivity * vertices[faces[tets[id].faces[l]].vertices[1]].temp + vertices[faces[tets[id].faces[l]].vertices[1]].thermalEmissivity_const
						+ vertices[faces[tets[id].faces[l]].vertices[2]].thermalEmissivity * vertices[faces[tets[id].faces[l]].vertices[2]].temp + vertices[faces[tets[id].faces[l]].vertices[2]].thermalEmissivity_const
						) / 3.0;
					///	以下の[]は上までの[l]と異なる。
					///	IDが何番目かによって、形状関数の係数が異なるので、
					tets[id].vecf[3] +=  ems_temp * Tout[j] * (1.0/3.0) * faces[tets[id].faces[l]].area * vecf4array[j];	//+=:j=0~3のvecf2arrayを加算								
				}
			}
		}
	}
...
double PHFemMeshThermo::CalcTetrahedraVolume2(unsigned id){
		...
		for(unsigned j =0; j < 4; j++){
			if(i == 0){
				tempMat44[i][j] = 1.0;
					...
					tempMat44[i][j] = vertices[tets[id].vertices[j]].pos.x;
			}
			else if(i == 2){
					tempMat44[i][j] = vertices[tets[id].vertices[j]].pos.y;
			}
			else if(i == 3){
					tempMat44[i][j] = vertices[tets[id].vertices[j]].pos.z;
			}
...
double PHFemMeshThermo::CalcTetrahedraVolume(Tet tet){
	PTM::TMatrixRow<4,4,double> tempMat44;
	for(unsigned i =0; i < 4; i++){
		for(unsigned j =0; j < 4; j++){
			if(i == 0){
				...
				tempMat44[i][j] = vertices[tet.vertices[j]].pos.x;
			}
			else if(i == 2){
				tempMat44[i][j] = vertices[tet.vertices[j]].pos.y;
			}
			else if(i == 3){
				tempMat44[i][j] = vertices[tet.vertices[j]].pos.z;
			}
...
void PHFemMeshThermo::CreateMatk3t_nonRadiantHeat(unsigned id){
	...
	for(unsigned l= 0 ; l < 4; l++){
		...
		if(tets[id].faces[l] < (int)nSurfaceFace && faces[tets[id].faces[l]].alphaUpdated){
			if(vertices[faces[tets[id].faces[l]].vertices[0]].beRadiantHeat && vertices[faces[tets[id].faces[l]].vertices[1]].beRadiantHeat 
				&& vertices[faces[tets[id].faces[l]].vertices[2]].beRadiantHeat	){
				...
				for(unsigned j=0;j<4;j++){
					if(tets[id].vertices[j] == ID){					///	形状関数が１、（すなわち）このfaceに対面する頂点　と一致したら　その時のfaceで面積分する
						...
						///	以下の[]は上までの[l]と異なる。
						///	IDが何番目かによって、形状関数の係数が異なるので、
						tets[id].matk[2] += ems_temp * (1.0/12.0) * faces[tets[id].faces[l]].area * matk3array[j];
					}
				}
			}
		}
...
//void PHFemMeshThermo::CreateMatk2t_(unsigned id){
//
//	//l=0の時k21,1の時:k22, 2の時:k23, 3の時:k24	を生成
//	for(unsigned l= 0 ; l < 4; l++){
//		//matk2array[l] = matk2temp;
...
bool PHFemMeshThermo::SetConcentricHeatMap(std::vector<double> r, std::vector<double> temp, Vec2d origin){
	//中心からの距離を求める
	double dx = 0.0;
	double dz = 0.0;
	for(unsigned i = 0; i < vertices.size(); ++i){
		if(vertices[i].disFromOrigin == 0){ 
			dx = vertices[i].pos.x - origin[0];
			dz = vertices[i].pos.z - origin[1];
			vertices[i].disFromOrigin = sqrt(dx * dx + dz * dz);
		}
	}
	//	ここまで
	//> 表面faceの内、原点から各faceの節点のローカル(x,z)座標系での平面上の距離の計算を、faceの全節点のy座標が負のものに対して、IH加熱の可能性を示すフラグを設定
	...
	for(unsigned k=0; k < vertices.size(); ++k){
		for(size_t i=0; i < r.size()-1;++i){
			...
			//}
				...
				if(vertices[k].disFromOrigin <=  (r[i] + r[i+1]) /2.0){
					vertices[k].temp = temp[i];
				} 
			}
		} 
...
double PHFemMeshThermo::Get_thConduct(){
	return thConduct;
}
void PHFemMeshThermo::SetThermalEmissivityToVtx(unsigned id,double thermalEmissivity,double thermalEmissivity_const){
	vertices[id].thermalEmissivity = thermalEmissivity;
	vertices[id].thermalEmissivity_const = thermalEmissivity_const;
}
void PHFemMeshThermo::SetThermalEmissivityToVerticesAll(double thermalEmissivity,double thermalEmissivity_const){
	for(unsigned i =0; i < vertices.size(); i++){
		vertices[i].thermalEmissivity = thermalEmissivity;
		vertices[i].thermalEmissivity_const = thermalEmissivity_const;
	}
}
void PHFemMeshThermo::SetHeatTransRatioToAllVertex(double heatTransR_){
	for(unsigned i =0; i < vertices.size() ; i++){
...
void PHFemMeshThermo::CreateTempVertex(){		//Resize and Set Temp to TVecAll 
	TVecAll.resize(vertices.size());
	SetTempAllToTVecAll((unsigned)vertices.size());
}
