	...
	if (f->shape){
		...
		for(int i=0; i<frames.size(); ++i){
			if (frames[i]->shape == sh){
				DSTR << sh->GetName() << "warning : 【PHSolid::AddFrame(CDShapeIf* shape)】Tried Adding the same shape twice. Skip registration." << std::endl;
				return;
			}
		}
	}
...
void PHSolid::AddShape(CDShapeIf* shape){
	...
	for(int i=0; i<frames.size(); ++i){
		if (frames[i]->shape == sh){
			DSTR << sh->GetName() << "warning : 【PHSolid::AddShape(CDShapeIf* shape)】Tried Adding the same shape twice. Skip registration." << std::endl;
			return;
		}
	}
