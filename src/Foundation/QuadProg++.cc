...
File $Id: QuadProg++.cc 232 2007-06-21 12:29:00Z digasper $
 ...
 2017.05.11 Modified for Springhead2 by Shoichi Hasegawa
*/
...
namespace Spr {
	...
	void compute_d(VVector< double >& d, const VMatrixRow< double >& J, const VVector< double >& np);
	void update_z(VVector<double>& z, const VMatrixRow<double>& J, const VVector<double>& d, int iq);
	void update_r(const VMatrixRow<double>& R, VVector<double>& r, const VVector<double>& d, int iq);
	bool add_constraint(VMatrixRow<double>& R, VMatrixRow<double>& J, VVector<double>& d, int& iq, double& rnorm);
	void delete_constraint(VMatrixRow<double>& R, VMatrixRow<double>& J, VVector<int>& A, VVector<double>& u, int n, int p, int& iq, int l);
	...
	void forward_elimination(const VMatrixRow<double>& L, VVector<double>& y, const VVector<double>& b);
	void backward_elimination(const VMatrixRow<double>& U, VVector<double>& x, const VVector<double>& y);

	// Utility functions for computing the scalar product and the euclidean 
	// distance between two numbers
	double scalar_product(const VVector<double>& x, const VVector<double>& y);
	double distance(double a, double b);

	// Utility functions for printing vectors and matrices
	void print_matrix(char* name, const VMatrixRow<double>& A, int n = -1, int m = -1);

	template<typename T>
	void print_vector(char* name, const VVector<T>& v, int n = -1);

	// The Solving function, implementing the Goldfarb-Idnani method

	double solve_quadprog(VMatrixRow<double>& G, VVector<double>& g0,
		const VMatrixRow<double>& CE, const VVector<double>& ce0,
		const VMatrixRow<double>& CI, const VVector<double>& ci0,
		VVector<double>& x)
	{
		...
		if (G.height() != n)
		{
			...
			DSTR << msg.str() << std::endl;
			throw std::logic_error(msg.str());
		}
		if (CE.height() != n)
		{
			msg << "The matrix CE is incompatible (incorrect number of rows " << CE.height() << " , expecting " << n << ")";
			DSTR << msg.str() << std::endl;
			throw std::logic_error(msg.str());
		}
		if (ce0.size() != p)
		...
		if (CI.height() != n)
		{
			msg << "The matrix CI is incompatible (incorrect number of rows " << CI.height() << " , expecting " << n << ")";
			DSTR << msg.str() << std::endl;
			throw std::logic_error(msg.str());
		}
		if (ci0.size() != m)
		...
		VMatrixRow<double> R, J;
		VVector<double> s, z, r, d, np, u, x_old, u_old;
		R.resize(n, n), J.resize(n, n);
		s.resize(m + p), z.resize(n), r.resize(m + p), d.resize(n), np.resize(n), u.resize(m + p), x_old.resize(n), u_old.resize(m + p);
		...
		VVector<int> A, A_old, iai;
		A.resize(m + p), A_old.resize(m + p), iai.resize(m + p);
		int q, iq, iter = 0;
		VVector<bool> iaexcl;
		iaexcl.resize(m + p);
	...
	void print_matrix(char* name, const VMatrixRow<double>& A, int n, int m)
	{
		std::ostringstream s;
		std::string t;
		if (n == -1)
	...
	template<typename T>
	void print_vector(char* name, const VVector<T>& v, int n)
	{
		std::ostringstream s;
		std::string t;
		if (n == -1)
