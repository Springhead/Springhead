...
そこで，ディスクリプタの宣言を読んで，型の情報を登録するC++のソースコードを生成するツール(typedesc.exe)
を使って，型情報を実行時に登録しています．
typedesc.exeのソースは， /bin/src/typedesc/ にあります．
Foundation, Physics, Graphics のライブラリをビルドすると，コンパイル前に
<pre>
	/src/Foundation/MakeTypeDesc.bat SDK名
</pre>
が実行されて typedesc.exe を使って型情報登録ソースコード 
TypeDescDumpImp.h TypeDescDump.h を生成してくれます．
XXRegisterTypeDesc.cpp がこれらをインクルードして，型情報を
UTTypeDescDb::theTypeDescDb に登録します．
型情報は，UTTypeDescオブジェクトで表されます．

ディスクリプタの宣言の記法がtypedescの想定外だと TypeDescDumpImp.h でエラーが起こることがあります．
そのような場合は，他のディスクリプタを参考に簡単な記法に直してください．
*/
