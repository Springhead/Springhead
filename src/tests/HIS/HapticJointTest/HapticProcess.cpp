...
HapticProcess::HapticProcess(){
	dt = 0.001f;
	K = 30;
	D = 0.001;		// ダンパを，0にしたら床のがたがたがなくなった．
	posScale = 100;
	bDisplayforce = false;
	bInter = true;
...
void HapticProcess::HapticRendering(){
	...
	if(bstack.grabObjectFlag==true){
		displayforce = -bstack.grabForce*bstack.grabForceRate;			// ユーザへの提示力
	}
	...
	for(unsigned i = 0; i < neighborObjects.size(); i++){
		...
		if(f < 0.0){																			// 内積が負なら力を計算
			...
			K = neighborObjects[i].phSolid.GetShape(0)->GetSpringK();			// 剛体ごとにばね係数を変える
			D = neighborObjects[i].phSolid.GetShape(0)->GetDamperD();			// 剛体ごとにダンパ係数を変える
...
//				proxyPos += (tanjent.norm() - abs(1.0 * posDot)) * tanjent.unit();
			...
			noContact = false;
				...
				//displayforce = bstack.grabForce;			// ユーザへの提示力
			}
