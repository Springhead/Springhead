...
Vec3f HISpidar2::motorPosDef[][4] =	//	モータの取り付け位置(中心を原点とする)
	{
...
HISpidar2::~HISpidar2(){SetMinForce();}

// links the HD to VD and RD
bool HISpidar2::Init(DVDeviceManager& dev){
	//	ドライバの取得
	int i;
	for(i=0; i<4; ++i){
...
bool HISpidar2::Init(DVDeviceManager& dev, Vec3f* motorPos, float vpn, float lpp, float minF, float maxF){
	for(int i=0; i<4; i++){
...
void HISpidar2::Update(float dt){
...
/*	2次計画法による張力計算	*/
void HISpidar2::SetForce(const Vec3f& v3force, float eff, float cont){
	//	糸の方向ベクトルを求める。
    for (int i=0; i<4; i++){
