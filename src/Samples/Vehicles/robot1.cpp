...
void Robot1::Build(PHSceneIf* scene, PHSdkIf* sdk){
	...
	PHHingeJointDesc jd;
	jd.pose[0].Pos() = Vec3d( 0.15, -0.1,  0.15);
...
void Robot1::Forward(){
	jntWheel[0]->SetDesiredVelocity(1.0);
	jntWheel[1]->SetDesiredVelocity(1.0);
	jntWheel[2]->SetDesiredVelocity(1.0);
	jntWheel[3]->SetDesiredVelocity(1.0);
}
