...
void guiSetDynamical(int control){
	animalBody->GetSolid(CRFourLegsAnimalBodyDesc::SO_WAIST)->SetDynamical((bool)dynamicalValue);
	glui->sync_live();
	...
	for(int i = 0; i<animalBody->NJoints(); i++){
		if(animalBody->GetJoint(i) != NULL){
			...
			if (ball){
				PHBallJointDesc d;
				jInfo->GetDesc(&d);
				if(solids[nowSolid] == animalBody->GetJoint(i)->GetChildObject(0)){
				...
				else if(solids[nowSolid] == animalBody->GetJoint(i)->GetChildObject(1)){				
...
void guiSetDataBallJoint(int control){
	...
	if(control == 0){				//< spinnerが更新された場合の処理
		if(DCAST(PHBallJointIf, animalBody->GetJoint(nowBallJoint))){
			...
			animalBody->GetJoint(nowBallJoint)->GetDesc(&ballDesc);
			ballDesc.spring			  = (double)ballJointData[0];
			ballDesc.damper			  = (double)ballJointData[1];
			animalBody->GetJoint(nowBallJoint)->SetDesc(&ballDesc);
		}
...
void guiSetDataHingeJoint(int control){
	...
	if(DCAST(PHHingeJointIf, animalBody->GetJoint(nowHingeJoint))){
		...
		animalBody->GetJoint(nowHingeJoint)->GetDesc(&hingeDesc);
		...
		animalBody->GetJoint(nowHingeJoint)->SetDesc(&hingeDesc);
	}
